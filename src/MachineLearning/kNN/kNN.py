...
def datingClassTest():
    ...
    m = norMat.shape[0]
    numTestVces = int(m * hoRatio)
    errorCount = 0.0
    for i in range(numTestVces):
        classifierResult = classify0(normMat[i:], normMat[numTestVces:m, :], datingLabels[numTestVces:m],3)
        print "the classifier came back with: %d, the real answer is: %d" % (classifierResult, datingLabels[i])
        if (classifierResult != datingLabels[i]):
            errorCount += 1.0
    print "the total error rate is : %f" % (errorCount / float(numTestVces))
